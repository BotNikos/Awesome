...
local selectorIcon = wibox.widget {
   {
      image = os.getenv ("HOME") .. '/.config/awesome/icons/feather_white/chevron-down.svg',
      forced_width = 24,
      forced_height = 24,
      halign = "center",
      widget = wibox.widget.imagebox
...
function updateProgress ()
   awful.spawn.easy_async_with_shell ("playerctl metadata -p " .. currentPlayer .. " --format '{{duration(position)}}|{{duration(mpris:length)}}'", function (out)
                                                 ...
                                                 elseif playerProgress.max_value >= 600 then
...
local buttonPrevious = wibox.widget {
   {
      image = os.getenv ("HOME") .. "/.config/awesome/icons/feather_white/skip-back.svg",
      halign = 'center',
      buttons = {
...
local buttonTogglePause = wibox.widget {
   {
      image = os.getenv ("HOME") .. "/.config/awesome/icons/feather_white/pause.svg",
...
local buttonNext = wibox.widget {
   {
      image = os.getenv ("HOME") .. "/.config/awesome/icons/feather_white/skip-forward.svg",
      forced_width = buttonSize,
      forced_height = buttonSize,
      halign = 'center',
      buttons = {
